...
class Flux_LoginServer extends Flux_BaseServer {
	 ...
	 *
	 */
	public function depositCredits($targetAccountID, $credits, $creditAmount = null)
	{
		...
		if (!$this->hasCreditsRecord($targetAccountID)) {
			...
			if (!is_null($creditAmount)) {
				$fields .= ', last_credit_date, last_credit_amount';
				$values .= ', NOW(), ?';
			}
			...
			if (!is_null($creditAmount)) {
				$sql .= ", last_credit_date = NOW(), last_credit_amount = ? ";
			}
