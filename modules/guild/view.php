...
$col .= "IF(ch.online = 1, 'En ligne actuellement', ";
$col .= "CASE DATE_FORMAT(acc.lastlogin, '%Y-%m-%d') ";
$col .= "WHEN DATE_FORMAT(NOW(), '%Y-%m-%d') THEN 'Aujourd''hui' ";
$col .= "WHEN DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 1 DAY), '%Y-%m-%d') THEN 'Hier' ";
$col .= "ELSE CONCAT('Il y a ', DATEDIFF(NOW(), acc.lastlogin), ' jours') ";
