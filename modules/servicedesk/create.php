...
$charselect=NULL;
if(!$charlist){
	$charselect='<option value="-1">Aucun personnage disponible</option>';
} else {
	$charselect='<option value="0">Tous les personnages</option>';
...
if(isset($_POST['account_id'])){
	...
	if(Flux::config('DiscordUseWebhook')) {
		if(Flux::config('DiscordSendOnNewTicket')) {
			sendtodiscord(Flux::config('DiscordWebhookURL'), 'Nouveau ticket créé : '. $subject);
		}
	}
	
	// Envoie un e-mail à tous les membres d'équipe qui ont activé les alertes.
	...
	if($staff){
		foreach($staff as $staffrow){
			...
			$sent = $mail->send($email, 'Nouveau ticket créé', 'newticket', array(
