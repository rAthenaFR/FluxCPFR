...
$catlist = $catsql->fetch();
$statusLabel = isset($statusLabels[$oldtrow->status]) ? $statusLabels[$oldtrow->status] : $oldtrow->status;
$lastReplyLabel = isset($lastReplyLabels[$oldtrow->lastreply]) ? $lastReplyLabels[$oldtrow->lastreply] : $oldtrow->lastreply;
...
$oldrowoutput.='<tr >
				...
				<td><a href="'. $this->url('servicedesk', 'view', array('ticketid' => $oldtrow->ticket_id)) .'" >
					'. $catlist->name .'</a></td>
				<td>
					<font color="'. Flux::config('Font'. $oldtrow->status .'Colour') .'"><strong>'. $statusLabel .'</strong></font>
				</td>
				<td width="50">';
					if($oldtrow->lastreply=='0'){$oldrowoutput.='<i>Aucun</i>';} else {$oldrowoutput.= $lastReplyLabel;}
$oldrowoutput.='</td>
				<td>
