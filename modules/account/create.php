	...
	try {
		...
		if ($result) {
			if (Flux::config('RequireEmailConfirm')) {
				...
				$sent = $mail->send($email, 'Confirmation du compte', 'confirm', array('AccountUsername' => $username, 'ConfirmationLink' => htmlspecialchars($link)));
				
				$createTable = Flux::config('FluxTables.AccountCreateTable');
				$bind = array($code);
				
				// Enregistrer le code de confirmation.
				...
				if ($sent) {
					...
					$discordMessage = 'E-mail de confirmation envoyé.';
				}
				else {
					$message  = Flux::message('AccountCreateFailed');
					$discordMessage = 'Échec de l’envoi de l’e-mail de confirmation.';
				}
				...
				$discordMessage = 'Compte créé.';
			}
			if(Flux::config('DiscordUseWebhook')) {
				if(Flux::config('DiscordSendOnRegister')) {
					sendtodiscord(Flux::config('DiscordWebhookURL'), 'Nouvelle inscription utilisateur : "'. $username . '" , ' . $discordMessage);
				}
			}
			$this->redirect();
		}
		else {
			exit('Erreur inattendue pendant la création du compte.');
		}
	}
