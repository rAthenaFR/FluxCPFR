...
<h3>Joueurs en ligne sur <?php echo htmlspecialchars($server->serverName) ?>.</h3>
<?php if ($auth->allowedToSearchWhosOnline): ?>
	...
	<form action="<?php echo $this->url ?>" method="get" class="search-form">
		...
		<p>
			...
			...
			<label for="guild_name">Guilde :</label>
...
<?php if ($hiddenCount): ?>
<p><?php echo number_format($hiddenCount) ?> joueur(s) ont choisi de se masquer de cette liste.</p>
<?php endif ?>

<table class="horizontal-table">
	<tr>
		<th><?php echo $paginator->sortableColumn('char_name', 'Nom du personnage') ?></th>
		<th>Classe job</th>
		<th><?php echo $paginator->sortableColumn('base_level', 'Niveau base') ?></th>
		<th><?php echo $paginator->sortableColumn('job_level', 'Niveau job') ?></th>
		<th colspan="2"><?php echo $paginator->sortableColumn('guild_name', 'Guilde') ?></th>
		<?php if ($auth->allowedToViewOnlinePosition): ?>
			<th><?php echo $paginator->sortableColumn('last_map', 'Carte') ?></th>
		<?php else: ?>
			<th>Carte</th>
		<?php endif ?>
	</tr>
	...
	<tr>
		...
		<?php else: ?>
			<span class="not-applicable">Masquée</span>
		<?php endif ?>
		</td>
	</tr>
	<?php endforeach ?>
...
<?php else: ?>
<p>Aucun personnage trouvé sur <?php echo htmlspecialchars($server->serverName) ?>. <a href="javascript:history.go(-1)">Retour</a>.</p>
<?php endif ?>
