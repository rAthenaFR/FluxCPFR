	...
	<tr>
		...
		<td bgcolor="#e1eaf3" valign="middle">
			<span style="display: inline-block; margin: 2px 2px 2px 0">
				Vous êtes connecté avec <strong><a href="<?php echo $this->url('account', 'view') ?>" title="Voir le compte"><?php echo htmlspecialchars($session->account->userid) ?></a></strong>
				sur <?php echo htmlspecialchars($session->serverName) ?>.
				
			<?php if (count($athenaServerNames=$session->getAthenaServerNames()) > 1): ?>
				Votre serveur préféré est :
