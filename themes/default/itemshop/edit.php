...
<h3>Modifier un objet dans la boutique</h3>
<?php if ($item): ?>
...
<table class="vertical-table">
	...
	<tr>
		...
		<td>
			...
			<label>Essayer d’utiliser l’image existante de l’objet ? <input type="checkbox" name="use_existing" value="1"<?php if ($item->shop_item_use_existing) echo ' checked="checked"' ?> /></label>
			<?php if ($image=$this->shopItemImage($item->shop_item_id)): ?>
			<p>
				Image actuelle :
				<?php if ($auth->actionAllowed('itemshop', 'imagedel')): ?>
					<a href="<?php echo $this->url('itemshop', 'imagedel', array('id' => $item->shop_item_id)) ?>">(Supprimer)</a>
				<?php endif ?>
			</p>
...
<?php else: ?>
<p>Impossible de modifier un objet inconnu dans la boutique. <a href="javascript:history.go(-1)">Retour</a>.</p>
<?php endif ?>
