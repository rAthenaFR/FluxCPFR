	...
	<table class="horizontal-table" width="100%">  
		...
		<?php foreach($news as $nrow):?>
			<tr>
				...
				<td>
					<a href="<?php echo $this->url('news', 'edit', array('id' => $nrow->id)); ?>">Modifier</a> |
					<a href="<?php echo $this->url('news', 'delete', array('id' => $nrow->id)); ?>" onclick="return confirm('<?php echo htmlspecialchars(Flux::message('CMSConfirmDeleteLabel')) ?>');">Supprimer</a>
				</td>
			</tr>
		<?php endforeach;?>
	</table>
