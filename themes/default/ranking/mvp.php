...
<?php if ($mvpdata): ?>
    <?php if($kills):?>
    <h3><?php echo Flux::config('MVPRankingLimit') ?> dernières éliminations</h3>
    		...
    		<td align="center">
    			<?php if ($kill->kill_char_id): ?>
    				<?php if ($auth->actionAllowed('character', 'view') && $auth->allowedToViewCharacter): ?>
    					<?php echo $this->linkToCharacter($kill->kill_char_id, $char_ids[$kill->kill_char_id] ? htmlspecialchars($char_ids[$kill->kill_char_id]['name']) : "Personnage supprimé") ?>
    				<?php else: ?>
    					<?php echo $char_ids[$kill->kill_char_id] ? htmlspecialchars($char_ids[$kill->kill_char_id]['name']) : "Personnage supprimé" ?>
    				<?php endif ?>
    			<?php else: ?>
    	...
    	<tr>
    		...
    		<td align="center">
    			<?php if ($mvp->kill_char_id): ?>
    				<?php if ($auth->actionAllowed('character', 'view') && $auth->allowedToViewCharacter): ?>
    					<?php echo $this->linkToCharacter($mvp->kill_char_id, $char_ids[$mvp->kill_char_id] ? htmlspecialchars($char_ids[$mvp->kill_char_id]['name']) : "Personnage supprimé") ?>
    				<?php else: ?>
    					<?php echo $char_ids[$mvp->kill_char_id] ? htmlspecialchars($char_ids[$mvp->kill_char_id]['name']) : "Personnage supprimé" ?>
    				<?php endif ?>
    			<?php else: ?>
    				<span class="not-applicable"><?php echo htmlspecialchars(Flux::message('NoneLabel')) ?></span>
    			<?php endif ?>
